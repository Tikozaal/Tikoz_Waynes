...
menucoffre = {
    ...
    Events = {
        onSelected = function(self, _, btn, PMenu, menuData, result)
            ...
            ESX.TriggerServerCallback("Tikoz:Inventairewaynes", function(inventory) 
                ...
                for i=1, #inventory.items, 1 do 
                    ...
                    if btn.name == "~s~"..item.label then
                        ...
                        TriggerServerEvent('Tikoz:CoffreDeposewaynes', item.name, tonumber(count))
                        OpenMenu("Menu :")
                    end
                end
            end)

            ESX.TriggerServerCallback("Tikoz:CoffreSocietywaynes", function(items)
               ...
               if btn.name == "Retiré" then
                ...
                for i=1, #itemstock, 1 do 
                    ...
                    if btn.name == itemstock[i].label then
                        ...
                        count = Keyboardput("Combien voulez vous déposé ? ", "", 15)
                        TriggerServerEvent('Tikoz:RetireCoffrewaynes', itemstock[i].name, tonumber(count), itemLabel)
                        OpenMenu("Menu :")
                    end
