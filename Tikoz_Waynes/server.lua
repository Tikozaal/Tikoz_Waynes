...
-- RegisterServerEvent("Tikoz:CommandeWaynes")
-- AddEventHandler("Tikoz:CommandeWaynes", function(prix, label)
--     local _source = source
--     local xPlayer = ESX.GetPlayerFromId(_source)
...
-- 			--TriggerClientEvent('Tikoz:Livreuroajzd', _source)
-- 		else
...
-- RegisterServerEvent("Tikoz:RecupCommandeWaynes")
-- AddEventHandler("Tikoz:RecupCommandeWaynes", function(label)
--     local _source = source
--     local xPlayer = ESX.GetPlayerFromId(_source)
...
-- RegisterServerEvent("Tikoz:AddHistoriqueWaynes")
-- AddEventHandler("Tikoz:AddHistoriqueWaynes", function(employe, prix, label, name)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)
--     local date = os.date('~b~%d/%m/%Y %H:%M')

-- 			MySQL.Async.execute('INSERT INTO tikoz_wayneshistorique (id, employe, date, name, prix) VALUES (@id, @employe, @date, @name, @prix)', {
-- 				["@id"] = id,
-- 				['@employe'] = employe,
-- 				['@name'] = name,
--                 ['@date'] = date,
...
-- RegisterServerEvent("Tikoz:waynesHistoriqueSupp")
-- AddEventHandler("Tikoz:waynesHistoriqueSupp", function(id, itemName)
-- 	local _source = source
-- 	local xPlayer = ESX.GetPlayerFromId(_source)

-- 		MySQL.Async.fetchAll("DELETE FROM tikoz_wayneshistorique WHERE id = "..id, {
-- 			["@id"] = id
-- 		}, function()
-- 		end)
...
-- ESX.RegisterServerCallback('Tikoz:waynesHistorique', function(source, cb)

--     local xPlayer = ESX.GetPlayerFromId(source)
--     local wayneshistorique = {}


--     MySQL.Async.fetchAll('SELECT * FROM tikoz_wayneshistorique', {

--     }, function(result)

--         for i=1, #result, 1 do

--             table.insert(wayneshistorique, {
-- 				id = result[i].id,
-- 				employe = result[i].employe,
--                 name = result[i].name,
--                 prix = result[i].prix,
--                 date = result[i].date,
--             })
